...
public class Main{
    ...
    static class MyType{
        int count,min,max;//bst的结点个数，当前二叉树的最大值和最小值
        Node node;//bst的头结点
        public MyType(int c,int mi,int ma,Node n){
            this.count=c;
            this.min=mi;
            this.max=ma;
            this.node=n;
    ...
    public static MyType process(Node head){
        if(head==null){
            return new MyType(0,Integer.MAX_VALUE,Integer.MIN_VALUE,null);
        }
        MyType left=process(head.left);
        MyType right=process(head.right);
        
        int min=Math.min(head.value,Math.min(left.min,right.min));
        int max=Math.max(head.value,Math.max(left.max,right.max));
        
        int count=0;
        if(head.value>left.max && head.value<right.min
          && head.left==left.node && head.right==right.node){
            count=left.count+right.count+1;
            return new MyType(count,min,max,head);
        }else{
            if(left.count>right.count){
                return new MyType(left.count,min,max,left.node);
            }else return new MyType(right.count,min,max,right.node);
        }
